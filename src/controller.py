...
from src.config.params import GuiParameters, FuelParameters, GuiSides, FuelSides, MainParameters
from app.database import async_session
from app.crud import drivers as autisti_crud
from app.crud.erogations import create_erogazione
from app.schemas.erogations import ErogazioneCreate
...
class Controller:
    def __init__(self):
        ...
        self.sides = {}
        self.validated_autisti = {}
        self.validated_veicoli = {}
        self.view = MainWindow(self)

        # Non usiamo più una lista statica di tessere
        self.card_validated = False
        self._temp_validated_autista = None 
        self._temp_validated_veicolo = None
    ...
    async def prompt_for_vehicle(self, autista):
        ...
        async with async_session() as session:
            ...
            self._temp_validated_veicolo = veicolo
    ...
    async def register_erogation_record_from_validated(self, side_number: int):
        ...
        litri_erogati = pump_obj.pulserCounter / pump_obj.params.pulsesPerLiter
        litri_erogati = litri_erogati / pump_obj.params.calibration_factor

        # Se l'autista è stato validato per quel lato, si tratta di erogazione automatica
        if side_number in self.validated_autisti:
            autista = self.validated_autisti[side_number]
            modalita = "automatica"
            tessera = autista.tessera
            nome_compagnia = autista.nome_compagnia
            nome_autista = autista.nome_autista
        else:
            # Erogazione manuale: nessun autista validato
            modalita = "manuale"
            tessera = None
            nome_compagnia = None
            nome_autista = None

        # Gestione del veicolo (simile)
        if side_number in self.validated_veicoli:
            veicolo = self.validated_veicoli[side_number]
            id_veicolo = veicolo.id_veicolo
            nome_compagnia_veicolo = veicolo.nome_compagnia
            km_totali_veicolo = veicolo.km_totali_veicolo
        else:
            id_veicolo = None
            nome_compagnia_veicolo = None
            km_totali_veicolo = None

        # Calcola il prezzo totale (esempio: litri erogati * prezzo per litro)
        prezzo_totale = litri_erogati * pump_obj.params.price

        # Prepara i dati per la registrazione
        erogazione_data = ErogazioneCreate(
            tessera = tessera,
            ...
            prezzo_totale_erogazione = f"{prezzo_totale:.2f}"
        )

        # Inserisci il record nel DB
        async with async_session() as session:
            try:
                nuovo_record = await create_erogazione(session, erogazione_data)
                logging.info(f"[INFO]: Erogazione registrata con successo per lato {side_number}: {nuovo_record}")
                return nuovo_record
            except Exception as e:
                logging.error(f"[ERROR]: Errore nella registrazione dell'erogazione per lato {side_number}: {e}")
                return None
    ...
    def side_clicked(self, side_number):
        ...
        if self._temp_validated_autista is None:
            logging.warning("[WARNING]: Nessun autista validato per questo lato.")
            return

        # Associa i dati dell'autista validato al lato selezionato
        self.validated_autisti[side_number] = self._temp_validated_autista
        logging.info(f"[INFO]: Autista associato al lato {side_number}: {self._temp_validated_autista.tessera}")

        self.validated_veicoli[side_number] = self._temp_validated_veicolo
